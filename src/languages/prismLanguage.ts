...
const PRISM_TOKENS: Record<string, (value: string) => Token> = {
  ...
  "doctype-tag": token.builtin,
  "attr-name": token.property,
    ...
    return {
      lex: (text: string) => {
        if (!prism.languages[language])
          throw new Error(
            `Language '${language}' doesn't exist, or hasn't been loaded yet.`
          );

        const tokens: (string | PrismToken)[] = prism.tokenize(
          text,
        ...
        function addString(str: string, parent?: string) {
          ...
          for (let i = 0; i < parts.length; i++) {
            lines[lines.length - 1].push(
              parent && parent in PRISM_TOKENS
                ? PRISM_TOKENS[parent](parts[i])
                : token.plain(str)
            );
